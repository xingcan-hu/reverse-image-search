...
# 增量设计文档：签到与分享邀请模块（积分体系 v2）

目标：在现有 MVP（Next.js App Router + Clerk + Drizzle/Postgres + Stripe）基础上，新增：

- **每日签到**：每天签到送 `1` 积分，**有效期 7 天**（限时点数）。
- **分享邀请**：分享邀请链接，**每成功邀请 1 个新用户**，邀请人获得 `20` 积分，**永久有效**（永久点数）。

设计核心难点（必须满足）：

1) **双账户体系**：必须区分“永久点数”和“限时点数”。  
2) **扣费优先级**：消费时必须先扣“快过期的点数”，再扣“永久点数”。  
3) **归因追踪**：邀请关系建立与奖励发放需可追踪、可幂等、可防刷。

> 本文档为“可落地”的实现说明：包含页面/前端/后端/DB Schema Change/扣费逻辑/组件规范/验收清单。  
> 不写代码，但会给出建议的文件落点与 API 契约，便于直接 Vibe Coding。